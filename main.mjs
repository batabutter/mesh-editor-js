import { keys } from "./modules/keypress.mjs"
...
class Camera {
    ...
    /**
     * Creates a camera at the given Vec3
     * @param {Vec3} position - Point where the camera will be
     */
    ...
    translate_z(dz) {
        this.position = { x: this.position.x, y: this.position.y, z: this.position.z + dz };
    }
}
...
class Scene {
    ...
    /**
     * Renders the camera view of the meshes
     */
    cameraView() {
        for (const mesh of meshArr) {
            let projectedMesh = {
                verts: new Array(mesh.verts.length)
            };
            for (let i = 0; i < mesh.verts.length; i++) {
                //mesh.verts[i] = translate_z(rotate_y(mesh.verts[i], dtheta), dz)
                const transformation = { x: mesh.verts[i].x, y: mesh.verts[i].y, z: mesh.verts[i].z - this.camera.position.z };
                const clippedPoint = this.camera.clipPoint(transformation)

                projectedMesh.verts[i] = clippedPoint
            }
            //console.log("Drawing mesh at > ", projectedMesh);
...
const camera = new Camera({ x: 0, y: 0, z: -3 });
